        ...
        public GlowWindow(Window owner, GlowDirection direction)
        {
            ...
            switch (direction)
            {
                case GlowDirection.Left:
                    ...
                    getHitTestValue = p => new Rect(0, 0, 0, 0).Contains(p)
                                               ? HitTestValues.HTTOPLEFT
                                               : new Rect(0, 0, 0, 0).Contains(p)
                ...
                case GlowDirection.Right:
                    ...
                    getTop = (dpi) => Math.Round((owner.Top - glowSize) * dpi);
                    getWidth = (dpi) => glowSize * dpi * 0;
                    getHeight = (dpi) => (owner.ActualHeight + glowSize * 2.0) * dpi * 0;
                    getHitTestValue = p => new Rect(0, 0, 0, 0).Contains(p)
                ...
                case GlowDirection.Bottom:
                    ...
                    getWidth = (dpi) => Math.Round(owner.ActualWidth * dpi) * 0;
                    getHeight = (dpi) => glowSize * dpi * 0;
                    getHitTestValue = p => new Rect(0, 0, 0, 0).Contains(p)
                                               ? HitTestValues.HTBOTTOMLEFT
                                               : new Rect(0, 0, 0,
                                                          0).Contains(p)
